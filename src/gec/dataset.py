...
def load_gec_dataset(dataset_name: str, size: Optional[int] = None) -> Dataset:
    ...
    for item in ds:
        ...
        applied_edits = item.get("applied_edits", 0)
        ...
        if user_msg:
            ...
            applied_edits_list.append(applied_edits)
    ...
    return Dataset.from_dict({
        ...
        "is_clean": is_clean_flags,
        "source": sources,
        "applied_edits": applied_edits_list,
    })
...
def make_gec_messages(incorrect_text: str, system_prompt: str) -> list[dict[str, str]]:
    ...
    return [
        ...
        {"role": "user", "content": f"Fix spelling, grammar, and punctuation. Return only the corrected text:\n<Text>\n{incorrect_text}\n</Text>"},
    ]
